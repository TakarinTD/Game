#include "Card.h"
Card::Card(std::shared_ptr<Models> model, std::shared_ptr<Shaders> shader, std::shared_ptr<Texture> texture,int x,int y) :Sprite2D(model, shader, texture)
{
	m_pBtClick = nullptr;
	m_isHandle = false;
	posX = x;
	posY = y;
	isSolve = false, Solving=false;
}
Card::~Card()
{
}
